...
class ConvAE(object):
    def __init__(self, n_input, kernel_size, n_hidden, reg_constant1 = 1.0, re_constant2 = 1.0, re_constant3 = 1.0, batch_size = 200, reg = None, \
        ...
        self.optimizer2 = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.loss2)  # GradientDescentOptimizer #AdamOptimizer
    ...
    def _initialize_weights(self):
        ...
        all_weights['Coef']   = tf.Variable(0.000001 * tf.random_normal([self.batch_size, self.batch_size],dtype=tf.float32), name = 'Coef')
...
def test_face(Img, Label, CAE, num_class):       
    ...
    for i in range(0,41-num_class): 
        ...
        max_step = 4000#50 + num_class*25# 100+num_class*20
        display_step = 50#max_step/20#10
        lr = 1.0e-3
        fine_step = 700
        # fine-tune network
        epoch = 0
        ...
        while epoch < max_step:
            ...
            if COLD is not None:
                ...
                if r < 1.0e-9 and lastr < 1.0e-9 and epoch >  fine_step:
                    print("early stop")
                    print("epoch: %.1d" % epoch, "cost: %.8f" % (cost[0] / float(batch_size)))
                    print(cost)
                    Coef = thrC(Coef, alpha)
